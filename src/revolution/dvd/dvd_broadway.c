...
#define CHECK_ALIGN_32(addr) (((u32)(addr) & 0x1F) == 0)

CW_FORCE_STRINGS(dvd_broadway_c_0, dvdContexts);

static void nextCommandBuf(void);
static DVDLowContext* newContext(DVDLowCallback callback, callbackType_t type);

static void* ddrAllocAligned32(int size) {
    u8* lo, *hi;

    if (!CHECK_ALIGN_32(size)) {
        return 0;
    }
...
static void initDvdContexts() {
    ...
    for (i = 0; i < DVD_LOW_CTX_MAX; i++) {
        ...
        dvdContexts[i].callbackType = BOGUS_TYPE;
...
static DECOMP_INLINE DVDLowContext* newContext(DVDLowCallback callback,
                                               callbackType_t type) {
    s32 id;
    bool use = dvdContexts[freeDvdContext].inUse != FALSE;
...
BOOL DVDLowOpenPartition(u32 offset, ESTicket* eTicket, u32 certsSize,
    ...
    if (eTicket != NULL && !CHECK_ALIGN_32(eTicket)) {
        OSReport("(DVDLowOpenPartition) eTicket memory is unaligned\n");
        return FALSE;
    }

    if (certificates != NULL && !CHECK_ALIGN_32(certificates)) {
        OSReport("(DVDLowOpenPartition) certificates memory is unaligned\n");
        return FALSE;
    }

    if (tmd != NULL && !CHECK_ALIGN_32(tmd)) {
        // @bug Copypasta from previous log; meant to type "tmd memory is unaligned\n".
        OSReport("(DVDLowOpenPartition) certificates memory is unaligned\n");
        return FALSE;
    }
...
// NOTE: Unused
BOOL DVDLowOpenPartitionWithTmdAndTicket(u32 offset, ESTicket *eTicket, u32 numTmdBytes, ESTitleMeta *tmd, u32 numCertBytes, void* certificates, DVDLowCallback callback) {
    DVDLowContext* ctx;
    IPCResult result;
    ...
    if (certificates != 0 && !CHECK_ALIGN_32(certificates)) {
        return FALSE;
    }
    ...
    if(eTicket == NULL) {
        ...
        return FALSE;
    } else if(!CHECK_ALIGN_32(eTicket)) {
        OSReport("(%s) eTicket memory is unaligned\n", __FUNCTION__);
        return FALSE;
    }
...
BOOL DVDLowOpenPartitionWithTmdAndTicketView(
    ...
    if (eTicketView == 0) {
        ...
        return FALSE;
    } else if (!CHECK_ALIGN_32(eTicketView)) {
...
BOOL DVDLowGetNoDiscBufferSizes(u32 offset, u32* numTmdBytes, u32* numCertBytes, DVDLowCallback callback) {
    DVDLowContext* ctx;
    IPCResult result;
    ...
    if (!CHECK_ALIGN_32(numTmdBytes)) {
        OSReport("(%s) numTmdBytes memory is unaligned\n", __FUNCTION__);
        return FALSE;
    }

    if (!CHECK_ALIGN_32(numCertBytes)) {
...
BOOL DVDLowWaitForCoverClose(DVDLowCallback callback) {
    ...
    requestInProgress = TRUE;
    ctx = newContext(callback, COVER_CB);
    nextCommandBuf();
